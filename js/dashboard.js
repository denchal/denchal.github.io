document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const response = await fetch('https://opis-maker.vercel.app/api/user/history', {
      method: 'GET',
      headers: {
    ...
    if (history.length === 0) {
      ...
      history.forEach(item => {
        ...
        listItem.addEventListener('click', async () => {
          const descriptionResponse = await fetch(`https://opis-maker.vercel.app/api/user/history/${item._id}`, {
            method: 'GET',
            headers: {
